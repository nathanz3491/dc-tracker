...
打开 <http://127.0.0.1:8765/>，八个页面：

| 页面 | 用来干什么 |
|---|---|
| **Projects** | 全部字段的表格，每个值下面有下划线标出处等级，鼠标悬停（手机上点一下）看原句 |
| **Map** | 地图。气泡面积是有出处的装机容量，虚线空心圈表示没人报过容量 |
| **Capex** | 换一个轴看同一批数据：按**买方**统计产能，而不是按站点。顺带在这里合并重复行 |
| **Queue** | 待读的标题，以及抓不下来的站点 |
| **Coverage** | 字段覆盖率、必需项目清单、各类障碍卡住多少产能 |
| **Commands** | 命令面板，直接在网页上跑 |
| **Runs** | 跑过的命令和它们的完整输出 |
| **Help** | 三个核心概念（出处等级、五条轨道、置信度）和什么操作要花钱 |
...
- **花钱的命令**（`sync`、`enrich`、`infer`、`search`、`ingest crawl`、`ingest edgar`）
  要把命令名原样打一遍才肯跑，误点不会花钱。
- **会删数据的命令**只有 `merge` 一个。它一分钱不花，但删掉的行找不回来，所以用的是
  同一道确认，理由不同。这道关卡认的是命令名，不看参数——看参数的关卡等于留了个后门。
- Capex 页面上的**重复行合并**：同一个园区常常被存了好几遍（建的、持有的、用的各算一行），
  在站点列表里只是碍眼，一旦按买方汇总就是个错数字——Abilene 那个园区在库里有四份，
  1.2 GW 被算了四次。选哪一行留下不影响数值：合并会把所有引用挪到留下的那行，
  然后按全部引用重算每个字段，你选的只是保留哪个行号。
...
tracker cloudflare --check     # 先看看条件齐不齐
tracker cloudflare             # 发布
```

会打印一个 `https://xxx.trycloudflare.com` 的公网地址。命令停掉，地址立刻失效。
（`tracker serve --tunnel` 是同一件事的简写版，两条命令跑的是同一段代码。）

控制台本身**还是只绑定 `127.0.0.1`**。是 `cloudflared` 主动向 Cloudflare 建了一条**出站**连接，
再把流量顺着这条连接送回来——所以你的网络上没有开任何端口，路由器和防火墙都不用动。

**两种形态：**

| 形态 | 命令 | 地址 |
|---|---|---|
| 临时隧道 | `tracker cloudflare` | 随机 `*.trycloudflare.com`，不需要 Cloudflare 账号，每次都换 |
| 具名隧道 | `tracker cloudflare --name X --hostname Y` | 你自己域名下的固定地址，重启也不变 |

要发给别人看就用具名隧道：临时地址每次都变，结果不是天天重发一遍，就是被记在某个不该记的地方。
具名隧道**不会由这条命令替你创建**——那要往你的家目录写凭据、往你的 DNS 区域写一条记录，
两样都比这个进程活得久，所以留给你自己跑：
...
`--check` 值得先跑一次：它会检查密码、`cloudflared` 在不在**并且真的能跑起来**、
具名隧道存不存在、数据库和前端文件齐不齐，然后退出。之所以要检查"能不能跑起来"，
是因为下载不完整的 `cloudflared.exe` 是一个头部合法的 PE 文件，
启动时报 WinError 193 且一个字都不打印——在需要这个链接的时候才发现，是最糟的时机。

**如果你在用代理**（很可能），这里有个坑，命令已经替你绕过去了：cloudflared 向 Cloudflare
申请临时隧道的那一个请求，用的是它自己构造的 HTTP 客户端，**不读 `HTTPS_PROXY`**——
curl、pip、浏览器全都走代理，唯独这个请求直连。在受限的网络上这就是能不能用的分水岭：
实测一小时内直连耗时在 3.8 秒到 28 秒之间浮动，走代理稳定在 4 秒左右，
而 cloudflared 给这个请求的预算大约只有 10 秒——所以四次里大概三次报
`context deadline exceeded`。

现在 `tracker cloudflare` 会在本地起一个中转，用 `--quick-service` 指给 cloudflared，
把这一个请求经代理转发出去。对外那一段仍然是到 Cloudflare 的 TLS，明文只存在于本机回环里，
而 cloudflared 和控制台本来就在那里通信。这个中转**不是通用代理**：目标主机是写死的常量，
能变的只有路径，绝对 URI 直接拒绝，隧道停它就停。

代理从环境变量里找，Windows 上还会读注册表的 `Internet Settings`——
Windows 程序都看那里，Go 程序不看。`--proxy http://主机:端口` 手动指定，
`--no-proxy` 完全不用中转，`--check` 会告诉你找到了哪个。

即使这样也会自动重试三次，因为这个失败本质是"这一分钟慢"而不是"被拒绝"。
真的一直不行就改用具名隧道，它根本不碰那个接口。
