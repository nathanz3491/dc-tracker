...
def logic() -> Canvas:
    ...
    c.box(
        ...
        ["1  drift repair, free. A row whose stored value its own sources no", "   longer support: re-running the declared policy is arithmetic, not", "   an opinion. The only mechanical fix in the whole report.", "", "2  answered by comparison, free. A read of data already stored,", "   never a judgement between two sourced figures. Three codes clear", "   that bar, and they were 448 of 536 resolvable findings.", "", "3  what is left goes to the agent (default), the older fixed menu", "   (--llm), or to you at the keyboard. --auto stops after step 2,", "   for scripts and for the console, which has no keyboard.", "", "A finding already answered on its row is not offered again. A ruling", "counts only if it moves the value the row holds: one naming a citation", "that states some other figure is refused, and a recorded X -> X reopens."],
        role="orange", title_size=13, align="left", sub_size=10.2,
    )
    c.box(
