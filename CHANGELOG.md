...
- **`docs/workflows/` — a diagram per staged command, and a rule that keeps it
  ...
  Four commands here are pipelines wearing a single name: `enrich`, `sync`,
  `duplicates` and `logic`. What their code explains well is what a stage *does*;
  what it explains badly is the order, what each stage may write, and where a run
  stops — which is exactly what an operator is asking when a phase was skipped, a
  budget was consumed by the first five of thirty projects, or a merge was refused
  after the model said "same site". Each now has a page led by a full-page diagram,
  followed by the flags, the measured numbers, and a source map naming the
  functions the diagram was drawn from.

  The diagrams are generated by a stdlib-only script rather than drawn, because the
  rule attached to them only holds if editing one is cheap: a diagram is a
  declarative block of boxes and arrows, and `python
  scripts/render_workflow_diagrams.py <name>` re-renders it. Colour carries the
  distinction the whole tool is organised around — teal is free and deterministic,
  grey-blue is one fetch, orange costs a model call, red is a refusal or a rail.
  The palette is measured out of the project's existing pipeline diagram so a
  reader meets one house style.

  Not Mermaid, though GitHub renders it: it lays itself out, and cannot express
  sectioned bands, a dashed frame around a sub-loop, or annotation panels beside the
  flow. The `.svg` files are committed against the usual rule about build output,
  because GitHub renders `![](enrich.svg)` and runs no build step — an uncommitted
  diagram is one nobody reading the repo can see.

  `CLAUDE.md` §7 makes updating the matching page part of the same change as the
  code, with a table from touched module to page. A stale diagram of a command that
  spends money and deletes rows is not a neutral omission; it is read as the current
  design.
