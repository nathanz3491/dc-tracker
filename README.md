...
3,320 tests, about three minutes. **A fresh clone with no API key and no network access
...
network anyway fails and names the host: `tests/conftest.py` refuses every DNS
lookup and connection past loopback, with the proxy variables cleared so a local
proxy cannot hide one. Each test also runs with a temporary `TRACKER_HOME`, so no
cache it writes lands in the checkout for a later run to be served.
